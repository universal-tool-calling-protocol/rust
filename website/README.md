...
The website workflow validates the content, builds a GitHub Pages artifact, and
publishes successful builds from the repository's default branch. Pull requests
run the same checks and build without deploying.

To enable publishing, open the repository's **Settings → Pages → Build and
deployment** and set **Source** to **GitHub Actions**. Push the workflow to the
default branch, or select **Actions → Website → Run workflow** on that branch.
The deployment URL appears in the workflow's `github-pages` environment.
`npm run dev` starts a local preview; GitHub Pages serves the files generated by
`npm run build`.
